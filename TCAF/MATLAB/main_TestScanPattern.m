%main_testScanPattern
% [nominalWTP, valid, validity, offTerMap] = ScanMapGetWTP(predTraj)
% ScanMapGetWTP 지형지물 탐색 및 WTP 생성
% 입력받은 예측 비행경로를 이용해 지형지물 정보를 탐색하고 WCP를 생성한다
%
%   [nominalWTP, valid] = ScanMapGetWTP(predTraj, 1)
%
% INPUT
...
if ON_PLOT
    ...
    plot(scanned(:,2), scanned(:,1), 'ro', 'MarkerSize', 4);
    xmin = min(predTraj(:,1,2), [], 'all') - 0.05;
    xmax = max(predTraj(:,1,2), [], 'all') + 0.05;
    ymin = min(predTraj(:,1,1), [], 'all') - 0.05;
    ymax = max(predTraj(:,1,1), [], 'all') + 0.05;
%     xmin = predTraj(1,1,2) - 0.075;
%     xmax = predTraj(1,1,2) + 0.01;
%     ymin = predTraj(1,1,1) - 0.005;
%     ymax = predTraj(1,1,1) + 0.005;
    grid minor;
    set(gca,'ytick', floor( min(predTraj(:,1,1))):1/240:ceil(max(predTraj(:,1,1))) );
    set(gca,'xtick', floor( min(predTraj(:,1,2))):1/240:ceil(max(predTraj(:,1,2))) );
    xlabel('Longitude [deg]'); ylabel('Latitude [deg]');
    ...
    for ii = 1:2:length(predTraj(:,1,2))
        h = plot(predTraj(ii, [2, 3], 2), predTraj(ii, [2, 3], 1), '-k');
        h.Annotation.LegendInformation.IconDisplayStyle = 'off';
    end
    legend('Start point', 'Projected flight path', 'Roll-recovered path', 'Roll-maintained path', 'Map Post', 'Location', 'best');
end
% -------------------------------------------------------------------------
