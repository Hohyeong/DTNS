...
% hDEM = out.TRNError.signals(4).values;
% time = out.tout;
% a=area(time,hDEM);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Flat 1~2%, Smooth 2~5%, Moderate 5~10%, Rough 10~20%                           %
%                                                                                %
...
TRNHorErr = median(sort(abs(out.TRNError.signals(1).values)));
TRNVerErr = median(sort(abs(out.TRNError.signals(2).values)));
...
if (any(GPSDataNotValid) == 0) && (any(Pcode) == 0)         % GPS C/A code
    if roughness < 2                                  % Flat
        if TRNHorErr < 50
            fprintf("TRNHorTest[CEP] result\n standard 50m, test %fm[Pass]\n",TRNHorErr);
        else
...
if (any(GPSDataNotValid) == 0) && (any(Pcode) == 1)   % GPS P code
